      ...
      <div className="bg-white dark:bg-zinc-900 w-full max-sm:max-w-xs max-w-sm rounded-[40px] shadow-2xl overflow-hidden animate-pop-in">
        <div className="bg-emerald-600 p-6 text-white flex justify-between items-center">
          <div>
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest opacity-80 mt-1">Matemática Sustentável</p>
          </div>
        ...
        <div className="p-6 space-y-4">
          <div className="bg-zinc-50 dark:bg-zinc-800/50 p-5 rounded-3xl border border-zinc-100 dark:border-zinc-800">
            <div className="space-y-3">
              <div className="flex justify-between items-center text-emerald-600">
                <span className="text-xs font-black uppercase tracking-widest flex items-center gap-2"><Leaf size={14} /> Mix Limpo ({currentHand.cleanCount}x)</span>
                <span className="font-black">+{currentHand.cleanCount * 15}</span>
              </div>
              <div className="flex justify-between items-center text-red-500">
                <span className="text-xs font-black uppercase tracking-widest flex items-center gap-2"><Factory size={14} /> Mix Fóssil ({currentHand.fossilCount}x)</span>
          ...
          <p className="text-[10px] text-zinc-400 font-bold uppercase text-center leading-tight">
            Combos anulam pontos base e atribuem bónus fixos.
          </p>
        </div>
      </div>
    </div>
    ...
    <>
      ...
      <div className={`fixed top-0 left-0 h-full w-64 bg-white dark:bg-zinc-950 z-[101] shadow-2xl transition-transform duration-500 border-r border-zinc-200 dark:border-zinc-800 p-6 pt-safe flex flex-col ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="mt-8 pt-4 border-t border-zinc-100 dark:border-zinc-800">
           <p className="text-[8px] font-bold text-zinc-400 uppercase text-center">EIA-ECOPOKER Pro v1.4</p>
        </div>
      </div>
    </>
  );
...
const App: React.FC = () => {
    ...
    setGameState(prev => {
      ...
      if (nextIdx >= prev.players.length) {
        ...
        if (nextRnd > prev.totalRounds) {
          ...
          // Guardar no Hall of Fame se terminou
    ...
    return (
      <div className="fixed inset-0 bg-emerald-950 flex flex-col items-center justify-center p-8 z-[120] text-white text-center">
        <div className="animate-float mb-8">
          <div className="w-24 h-24 bg-emerald-500 rounded-3xl mx-auto mb-6 flex items-center justify-center shadow-2xl shadow-emerald-500/50">
            <Trophy size={48} />
          </div>
        ...
        <button 
          onClick={() => { 
            setShowSplash(false); 
            audioService.resumeContext(); // Desbloqueia o som no primeiro clique
  ...
  if (gameState.isGameOver) {
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col p-6 text-white pb-safe pt-safe overflow-y-auto no-scrollbar">
        ...
        <div className="space-y-4 mb-10 flex-1">
          {sortedFinalPlayers.map((p, i) => (
            <div key={p.id} className={`relative overflow-hidden p-6 rounded-[35px] border transition-all animate-pop-in ${i === 0 ? 'bg-emerald-900/40 border-amber-500/50 scale-105 shadow-2xl' : 'bg-zinc-900 border-zinc-800'}`} style={{ animationDelay: `${i * 100}ms` }}>
              <div className="flex items-center justify-between relative z-10">
                <div className="flex items-center gap-5">
                  ...
                  <div className="flex flex-col">
                    <span className={`font-black text-xl uppercase tracking-tighter ${i === 0 ? 'text-white' : 'text-zinc-400'}`}>{p.name}</span>
                    {i === 0 && <span className="text-[9px] font-black text-amber-400 uppercase tracking-widest flex items-center gap-1 mt-0.5"><Trophy size={10} /> Líder Verde</span>}
                  </div>
                </div>
        ...
        <button onClick={resetGame} className="w-full bg-emerald-600 text-white font-black py-7 rounded-[35px] shadow-2xl uppercase tracking-widest text-sm active:scale-95 transition-all mb-8">Novo Ciclo Global</button>
      </div>
    );
  }
      ...
      return (
        <div className="space-y-6">
          <div className="bg-white dark:bg-zinc-900 p-8 rounded-[40px] shadow-2xl border border-zinc-100 dark:border-zinc-800">
            <h2 className="text-2xl font-black mb-6 italic flex items-center gap-3 uppercase"><Users className="text-emerald-500" /> Painel de Jogadores</h2>
            <div className="mb-8">
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between px-2">
          <div className="flex items-center gap-3">
            <button onClick={() => { setIsRankingOpen(true); audioService.playClick(); }} className="p-3 bg-white dark:bg-zinc-900 rounded-2xl shadow-md text-emerald-600 active:scale-90 transition-all">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-2xl font-black tracking-tighter italic uppercase leading-none truncate max-w-[140px]">{currentPlayer.name}</span>
            </div>
          </div>
          ...
          {isTurnFinished && !isRolling && (
            <div className="absolute top-4 left-0 right-0 z-40 flex justify-center animate-in slide-in-from-top-4 duration-500 px-4">
               <div className="bg-white/95 dark:bg-zinc-800/95 backdrop-blur-md px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 border border-emerald-500/30">
                  ...
                  <p className="font-black text-[10px] text-emerald-950 dark:text-white uppercase italic tracking-tighter whitespace-nowrap">Vez concluída! Confirma os resultados</p>
               </div>
            </div>
          )}
    ...
    <Layout activeTab={activeTab} setActiveTab={setActiveTab} gameStarted={!setupMode}>
      ...
      {activeTab === 'scores' && (
        <div className="space-y-6 animate-in fade-in duration-500">
          <div className="flex items-center gap-4 bg-zinc-100 dark:bg-zinc-900 p-1 rounded-[25px]">
            <button onClick={() => setRankingSubTab('current')} className={`flex-1 py-3 rounded-[20px] font-black text-xs uppercase tracking-widest transition-all ${rankingSubTab === 'current' ? 'bg-white dark:bg-zinc-800 shadow-md text-emerald-600' : 'text-zinc-400'}`}>Campanha</button>
      ...
      {activeTab === 'rules' && (
        <div className="space-y-8 p-2 pb-10 animate-in fade-in duration-500">
          ...
          <div className="space-y-6">
            <section className="bg-white dark:bg-zinc-900 p-6 rounded-[40px] shadow-xl space-y-4 border border-emerald-100 dark:border-zinc-800">
              ...
              <p className="text-[11px] text-zinc-500 dark:text-zinc-400 leading-relaxed font-medium">
                A combinação de maior valor é o <span className="text-emerald-600 font-black">Clean Power</span>, que ocorre quando os seis dados apresentam exatamente o mesmo símbolo de energia limpa (ex: 6 Solars). Representa um sistema totalmente homogéneo e 100% renovável.
              </p>
              <p className="text-[11px] text-zinc-500 dark:text-zinc-400 leading-relaxed font-medium">
                Logo abaixo surge o <span className="text-emerald-600 font-black">Clean Flush Total</span>: seis dados apenas de energias limpas em qualquer mix. Traduz um mix totalmente descarbonizado.
              </p>
            </section>
            ...
            <div className="grid gap-4">
               {[
                 ...
                 { name: "Three Clean!", desc: "Três dados iguais de energia limpa. Transição relevante.", pts: 60 },
                 ...
                 <div key={i} className="bg-white dark:bg-zinc-900 p-5 rounded-[30px] border border-zinc-100 dark:border-zinc-800 shadow-md flex flex-col gap-1">
            ...
            <section className="bg-red-50 dark:bg-red-950/20 p-6 rounded-[40px] border border-red-100 dark:border-red-900/50 space-y-4">
               ...
               <p className="text-[11px] text-zinc-500 dark:text-zinc-400 leading-relaxed">
                 <span className="text-red-600 font-black">Mixed Energy:</span> Mix sem combos. Cada fóssil penaliza (-20 pts) vs cada limpa (+15 pts).<br/><br/>
                 <span className="text-red-700 font-black">Dirty Energy:</span> Quatro ou mais dados fósseis. Sistema poluente (-100 pts).<br/><br/>
                 <span className="text-zinc-900 dark:text-white font-black">Blackout:</span> Seis dados exclusivamente fósseis. Dependência total, 0 pontos.
               </p>
            </section>
          </div>
        </div>
      ...
      {activeTab === 'settings' && (
        <div className="space-y-6 animate-in fade-in duration-500">
           <h2 className="text-3xl font-black italic tracking-tighter px-2 uppercase">Ajustes</h2>
           <div className="bg-white dark:bg-zinc-900 rounded-[45px] p-8 space-y-8 shadow-xl border border-zinc-100 dark:border-zinc-800">
              <div className="flex items-center justify-between">
                <span className="font-black text-sm uppercase tracking-widest flex items-center gap-2"><Volume2 size={18} /> Efeitos</span>
              ...
              <div className="flex items-center justify-between">
                <span className="font-black text-sm uppercase tracking-widest flex items-center gap-2"><Music size={18} /> Música</span>
